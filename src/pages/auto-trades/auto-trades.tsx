...
const BrandedYouTubeIcon = ({ size = 18 }: { size?: number }) => (
    <svg width={size} height={size} viewBox='0 0 24 24' fill='none' xmlns='http://www.w3.org/2000/svg'>
        <path
            ...
            fill='white'
        />
        <path d='M9.545 15.568V8.432L15.818 12l-6.273 3.568z' fill='#7a0000' />
    </svg>
);
...
const AutoTrades = observer(() => {
    ...
    const [journal, setJournal] = useState<
        { id: number; type: string; message: string; color: string; time: string }[]
    >([]);
    const [marketData, setMarketData] = useState<
        Record<string, { digit: number | string; price: string; history: number[] }>
    >({});
    ...
    const requestProposal = (symbol: string, contractType: string, barrier: string | null) => {
        ...
        if (
            barrier !== null &&
            (contractType.includes('OVER') || contractType.includes('UNDER') || contractType.includes('MATCH'))
        ) {
            req.barrier = barrier;
        }
            ...
            if (recoveryConditionType === 'rise' || recoveryConditionType === 'fall') {
                ...
                if (recoveryConditionType === 'odd') conditionMet = recent.every(d => d % 2 !== 0);
                    ...
                    conditionMet = recent.every(d =>
                        digitMatchesOperator(d, recoveryConditionOperator, recoveryConditionBarrier)
                    );
            }
    ...
    const handleContractResult = (contract: any) => {
        ...
        logJournal(
            'RESULT',
            `${contract.symbol} ${profit > 0 ? 'WIN' : 'LOSS'} (Net: $${profit.toFixed(2)})`,
        ...
        let reason = '';
        if (stateRef.current.totalProfitVal >= takeProfit) {
            stopTriggered = true;
            reason = 'TAKE PROFIT';
        } else if (stopType === 'amount') {
            if (stateRef.current.totalProfitVal <= -stopLoss) {
                stopTriggered = true;
                reason = 'STOP LOSS';
            }
        } else if (stopType === 'consec') {
            if (stateRef.current.consecutiveLosses >= consecutiveLossLimit) {
                stopTriggered = true;
                reason = 'LOSS LIMIT';
            }
        }
        ...
        if (stopTriggered) {
            logJournal('SYSTEM', `${reason} reached. Stopping.`, 'text-yellow-400');
            stopBot();
        }
    };
    ...
    const connect = () => {
        ...
        ws.onmessage = msg => {
            ...
            if (data.msg_type === 'authorize') {
                if (data.error) {
                    logJournal('ERROR', data.error.message, 'text-red-500');
                    setIsTrading(false);
                    return;
                }
            ...
            if (data.msg_type === 'asset_index') {
                Object.values(data.asset_index).forEach((cat: any) =>
                    cat.forEach((asset: any) => {
                        if (asset.symbol && typeof asset.pip_size === 'number')
                            stateRef.current.pipSizes.set(asset.symbol, asset.pip_size);
                    })
                );
            ...
            if (data.msg_type === 'tick') {
                ...
                tHist.push(digit);
                if (tHist.length > 10) tHist.shift();
                ...
                pHist.push(quote);
                if (pHist.length > 20) pHist.shift();
                updateMarketTile(symbol, digit, price, [...tHist]);
                if (stateRef.current.isTrading && !stateRef.current.isPaused && !stateRef.current.globalTradeLock)
                    checkLogic(symbol, digit);
            }
            ...
            if (data.msg_type === 'buy') {
                if (data.error) {
                    logJournal('ERROR', `Buy failed: ${data.error.message}`, 'text-red-500');
                    stateRef.current.pendingProposalSymbol = null;
                    stateRef.current.globalTradeLock = false;
                } else {
                    stateRef.current.activeContracts.add(data.buy.contract_id);
                    ws.send(
                        JSON.stringify({ proposal_open_contract: 1, contract_id: data.buy.contract_id, subscribe: 1 })
                    );
                }
            }
        ...
        ws.onclose = () => {
            stateRef.current.isAuthorized = false;
            if (stateRef.current.isTrading) setTimeout(connect, CONFIG.RECONNECT_DELAY);
        };
    };
    ...
    const clearJournalAndReset = () => {
        ...
        setLossCount(0);
        setIsRecoveryMode(false);
        stateRef.current.totalProfitVal = 0;
        stateRef.current.totalStakeVal = 0;
        stateRef.current.winCount = 0;
        stateRef.current.lossCount = 0;
        stateRef.current.consecutiveLosses = 0;
        stateRef.current.isRecovery = false;
        stateRef.current.recoveryAttempts = 0;
        stateRef.current.recoveryWinStreak = 0;
    ...
    useEffect(() => {
        if (wsRef.current && stateRef.current.isAuthorized) {
            wsRef.current.send(JSON.stringify({ forget_all: 'ticks' }));
            subscribeMarkets();
        }
    }, [selectedCategory]);
    ...
    return (
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className='auto-trades'>
            ...
            <section className='stats-grid'>
                <div className='glass-panel stat-card'>
                    <span className='label'>
                        <Wallet size={12} className='mr-1' /> Balance
                    </span>
                    <span className='value mono text-white'>${balance}</span>
                </div>
                <div className='glass-panel stat-card'>
                    <span className='label'>
                        <Activity size={12} className='mr-1' /> Stake
                    </span>
                ...
                <div className='glass-panel stat-card'>
                    ...
                    <span className={`value mono ${totalProfitVal >= 0 ? 'text-green-400' : 'text-red-400'}`}>
                        ${totalProfitVal.toFixed(2)}
                    </span>
                </div>
                <div className='glass-panel stat-card'>
                    <span className='label'>
                        <BarChart3 size={12} className='mr-1' /> Accuracy
                    </span>
            ...
            <motion.div layout className={`glass-panel logic-display ${isRecoveryMode ? 'glow-red' : 'glow-green'}`}>
            ...
            <div className='glass-panel controls-panel'>
                <div className='grid grid-cols-2 gap-3'>
                    <div className='control-group'>
                        <label>
                            <Target size={12} /> Strategy Architecture
                        </label>
                        <select
                            value={mainStrategy}
                    ...
                    <div className='control-group'>
                        <label>
                            <RotateCcw size={12} /> Recovery Engine
                        </label>
                        <div className='recovery-box'>
                            <div className='row'>
                                ...
                                <input
                                    type='number'
                                    value={recoveryTicks}
                                    onChange={e => setRecoveryTicks(parseInt(e.target.value) || 0)}
                                    className='mini-input'
                                />{' '}
                                TICKS =
                                <select
                                    value={recoveryConditionType}
                                    onChange={e => setRecoveryConditionType(e.target.value)}
                                ...
                                {recoveryConditionType.match(/over|under|equal/) && (
                                    <>
                                        <select
                                            value={recoveryConditionOperator}
                                            onChange={e => setRecoveryConditionOperator(e.target.value)}
                                            className='mini-select'
                                        >
                                            <option value='>'>&gt;</option>
                                            <option value='>='>&gt;=</option>
                                        ...
                                        <input
                                            type='number'
                                            value={recoveryConditionBarrier}
                                            onChange={e => setRecoveryConditionBarrier(parseInt(e.target.value))}
                                            className='mini-input'
                                        />
                                    </>
                                )}
                            ...
                            <div className='row'>
                                ...
                                {recoveryTradeType.match(/over|under|equal/) && (
                                    <input
                                        type='number'
                                        value={recoveryTradeBarrier}
                                        onChange={e => setRecoveryTradeBarrier(parseInt(e.target.value))}
                                        className='mini-input'
                                    />
                                )}
                            </div>
                ...
                <div className='settings-row'>
                    ...
                    <label className='switch'>
                        <input
                            type='checkbox'
                            checked={recoveryToggle}
                            onChange={e => setRecoveryToggle(e.target.checked)}
                        />
                ...
                <div className='grid grid-cols-2 gap-3'>
                    <div className='control-group'>
                        ...
                        <input
                            ...
                            step='0.01'
                            className='custom-select text-center font-bold'
                        />
                    </div>
                    <div className='control-group'>
                ...
                <div className='settings-row'>
                    ...
                    <label className='switch'>
                        <input
                            type='checkbox'
                            checked={martingaleToggle}
                            onChange={e => setMartingaleToggle(e.target.checked)}
                        />
                        <span className='slider'></span>
                    </label>
                </div>
                ...
                <div className='grid grid-cols-2 gap-3'>
                    <div className='control-group'>
                        <div className='flex items-center gap-2 mb-1'>
                            <input
                                type='radio'
                                checked={stopType === 'amount'}
                                onChange={() => setStopType('amount')}
                            />
                    ...
                    <div className='control-group'>
                        <div className='flex items-center gap-2 mb-1'>
                            <input
                                type='radio'
                                checked={stopType === 'consec'}
                                onChange={() => setStopType('consec')}
                            />
                ...
                <div className='control-group'>
                    ...
                    <input
                        ...
                        className='custom-select text-center font-bold text-green-400'
                    />
                </div>
                ...
                <div className='grid grid-cols-2 gap-4 mt-2'>
                    <button
                        onClick={
                            isTrading ? (isPaused ? () => setIsPaused(false) : () => setIsPaused(true)) : startTrading
                        }
                    ...
                    <button onClick={stopBot} disabled={!isTrading} className='btn-main stop'>
                        <Square size={18} /> Stop
                    </button>
                </div>
            </div>
            ...
            <section className='glass-panel market-box'>
                ...
                <div className='grid'>
                    {CATEGORIES[selectedCategory].map(sym => (
                        <div
                            key={sym}
                            className={`glass-panel tile ${marketData[sym]?.digit > 2 && marketData[sym]?.digit < 8 ? 'active shadow-blue-500/20' : ''}`}
                        >
                            ...
                            <div className='row-main'>
                                ...
                                <div className='hist'>
                                    {marketData[sym]?.history?.slice(-5).map((h, i) => (
                                        <span key={i} className={`mx-0.5 ${h > 5 ? 'text-green-500' : 'text-red-500'}`}>
                                            {h}
                                        </span>
                                    ))}
                                </div>
            ...
            <section className='journal-box'>
                <div className='header'>
                    <h4 className='title'>
                        <div className='dot animate-pulse'></div> Live Operations Journal
                    </h4>
                    <button onClick={clearJournalAndReset} className='reset-btn flex items-center gap-1'>
