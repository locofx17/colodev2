...
const DigitDistributionModal = observer(() => {
        ...
        if (tradeOptionsBlock && tradeTypeBlock) {
            ...
            if (predictionInput && predictionInput.connection && predictionInput.connection.targetBlock()) {
                ...
                if (targetBlock.type === 'math_number' || targetBlock.type === 'math_number_positive') {
                    ...
                    const setBlock = allBlocks.find(
                        b => b.type === 'variables_set' && b.getFieldValue('VAR') === varName
                    );
                    if (setBlock) {
    ...
    const detectPrecision = useCallback(
        (quote: number) => {
    ...
    const connect = useCallback(() => {
        ...
        ws.current.onopen = () => {
            ...
            ws.current?.send(
                JSON.stringify({ ticks_history: symbol, count: ticksRef.current, end: 'latest', style: 'ticks' })
            );
    ...
    const { prediction } = digit_stats_settings;
    const overLabel = `Over ${prediction}`;
    const underLabel = `Under ${prediction + 1}`;

    const overCount = digitFreq.reduce((acc, f, i) => (i > prediction ? acc + f : acc), 0);
    const underCount = digitFreq.reduce((acc, f, i) => (i <= prediction ? acc + f : acc), 0);
            ...
            <div className='digit-dist-modal__content'>
                <div className='digit-dist-modal__header'>
                    ...
                    <div className='digit-dist-modal__price-info'>
                        ...
                        <div className='digit-dist-modal__price-value'>
                            {pricePrefix}
                            <span className='digit-dist-modal__price-last'>{priceLastChar}</span>
                        </div>
                    </div>
                </div>
                ...
                <div className='digit-dist-modal__stats'>
                    {[
                        ...
                        { label: overLabel, val: formatPercent(overCount, total), compare: underCount },
                        { label: underLabel, val: formatPercent(underCount, total), compare: overCount },
                    ].map((s, i) => {
                        const isWinner = parseFloat(s.val) >= (s.compare / total) * 100;
                        let statClass = isWinner ? 'green' : 'red';
                        
                        // Use blue/yellow for Over/Under based on current prediction
                        if (s.label === overLabel || s.label === underLabel) {
