...
export const AccountSwitcherWalletItem = observer(
    ({ closeAccountsDialog, account, show_badge = false }: TAccountSwitcherWalletItemProps) => {
        ...
        const display_balance = dtrade_balance || 0;
            ...
            >
                ...
                <div className='acc-switcher-wallet-item__content'>
                    ...
                    <Text size='xs' weight='bold'>
                        {`${formatMoney(currency ?? '', display_balance, true)} ${getCurrencyDisplayCode(currency)}`}
                    </Text>

                </div>
                {show_badge && (
                    <WalletBadge
                        is_demo={Boolean(is_virtual) ? 'demo' : undefined}
