...
const SniperContent = observer(() => {
    ...
    const startScan = useCallback(() => {
        const markets =
            sniper.strategyLock === 'none' ? VOLATILITY_MARKETS.map(m => m.id) : VOLATILITY_MARKETS.map(m => m.id); // Or a specific subset
    ...
    return (
        <div className='sniper-content'>
            {!showSetup && (
                <header className='sniper-content__header'>
                    ...
                    <div className='sniper-content__header-actions'>
                        {sniper.isScanning && (
                            <button onClick={cancelScan} className='sniper-content__header-stop-btn' title='Stop Scan'>
                        ...
                        <div className='sniper-content__header-status-group'>
                            <button className='sniper-content__connect-btn' title='Manual Reconnect'>
                                {sniper.isScanning ? (
                                    <>
                                ...
                                <RefreshCw
                                    size={10}
                                    className={`refresh-icon ${!sniper.isScanning ? 'anim-spin' : ''}`}
                                />
                            </button>
            ...
            {showSetup ? (
                ...
                <main className='sniper-content__dashboard'>
                    ...
                    <div className='sniper-content__main-content'>
                        <div className='sniper-content__trade-dashboard'>
                            <div className='dashboard-grid'>
                                    ...
                                    <div className='status-main'>
                                        ...
                                        <div>
                                            ...
                                            <div className='status-desc'>
                                                {run_panel.is_running
                                                    ? `Stage: ${run_panel.contract_stage}`
                                                    : 'Sniper Intelligence standing by...'}
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                {/* Profit/Loss Card */}
                                <div
                                    className={`stats-card pl-card ${summary_card.profit_loss >= 0 ? 'profit' : 'loss'}`}
                                >
                                    <div className='card-header'>
                                    ...
                                    <div className='card-footer'>Current session active contract</div>
                                </div>
                                ...
                                <div className='stats-card summary-card-mini'>
                                    ...
                                    <div className='stats-main'>
                                        <div
                                            className={`value ${transactions.statistics.total_profit >= 0 ? 'text-green' : 'text-red'}`}
                                        >
                    ...
                    <div className='sniper-content__sidebar'>
                                        ...
                                        >
                                            ...
                                            <div className='signal-footer'>
                                                <div className='price-info'>
                                                    ...
                                                    <div className='value'>
                                                        {signal.price.toFixed(getMarketDecimals(signal.marketId))}
                                                    </div>
                                                </div>
                                                <button
                        ...
                        {showSetup && (
                            <div className='sniper-content__controls'>{/* ... (existing controls) ... */}</div>
                        )}
                    </div>
                </main>
            )}
            ...
            {!showSetup &&
                false && ( // Completely hidden per user request
                    <footer className='sniper-content__footer'>{/* ... footer content (now hidden) ... */}</footer>
                )}
        </div>
    );
});
